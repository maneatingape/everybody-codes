...
pub fn part2(notes: &str) -> usize {
    ...
    loop {
        ...
        let count: &mut u32 = seen.entry(key(&dance)).or_default();
        *count += 1;
...
fn shuffle(dance: &mut Dance, round: usize) {
    let dest = (round + 1) % 4;
    let size = dance[dest].len();

    let clapper = dance[round % 4].remove(0);
    let quotient = (clapper - 1) / size;
    let remainder = (clapper - 1) % size;

    let offset = if quotient % 2 == 0 { remainder } else { size - remainder };
...
fn shout(dance: &Dance) -> usize {
    let concat = format!("{}{}{}{}", dance[0][0], dance[1][0], dance[2][0], dance[3][0]);
    concat.parse().unwrap()
}
